...
export default function App(props) {
  ...
  return (
    <div className='App'>
      ...
      <div className="row">
        <div>
          <textarea value={cdi4body} onChange={(e) => { setCdi4Body(e.target.value) }}></textarea>
          <button onClick={() => { create_4(JSON.parse(cdi4body)) }}>Create</button>
          <button onClick={() => { update_4(JSON.parse(cdi4body)) }}>Update</button>
        </div>
        <div>
          <textarea value={cdi5body} onChange={(e) => { setCdi5Body(e.target.value) }}></textarea>
          <button onClick={() => { create_5(JSON.parse(cdi5body)) }}>Create</button>
          <button onClick={() => { update_5(JSON.parse(cdi5body)) }}>Update</button>
        </div>
        <div>
          <textarea value={v2body} onChange={(e) => { setV2Body(e.target.value) }}></textarea>
          <button onClick={() => { create_v2(JSON.parse(v2body)) }}>Create</button>
          <button onClick={() => { update_v2(JSON.parse(v2body)) }}>Update</button>
        </div>
      </div>
