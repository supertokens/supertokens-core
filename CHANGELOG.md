...
- Replaced the synthetic `user_last_active` activity event with semantic activity events (`sign_in`, `token_refresh`, `session_create`, `sign_out`, `oauth_token_exchange`, `oauth_authorize`); sign-up and created-new-user paths emit no activity event (the in-transaction `user_creation` lifecycle event records that activity). The last-active fold now reads these events plus `user_creation` and `account_linking`, and the `user_last_active` event type is retired.
