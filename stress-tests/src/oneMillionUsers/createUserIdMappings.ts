...
export const createUserIdMappings = async (users: { recipeUserId: string, email?: string, phoneNumber?: string }[]) => {
    ...
    await measureTime("Create user id mappings", async () => {
        await workInBatches(users.length, 8, async (idx) => {
            ...
            if (Math.random() < 0.5) {
                const newUserId = Array(64).fill(0).map(() => String.fromCharCode(97 + Math.floor(Math.random() * 26))).join('')
