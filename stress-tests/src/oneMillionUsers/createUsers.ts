...
const createEmailPasswordUsers = async () => {
    ...
    return await workInBatches(Math.floor(TOTAL_USERS / 5), 4, async (idx) => {
        const email = Array(64).fill(0).map(() => String.fromCharCode(97 + Math.floor(Math.random() * 26))).join('') + '@example.com';
...
const createThirdPartyUsers = async (thirdPartyId: string) => {
    ...
    return await workInBatches(Math.floor(TOTAL_USERS / 5), 4, async (idx) => {
        const email = Array(64).fill(0).map(() => String.fromCharCode(97 + Math.floor(Math.random() * 26))).join('') + '@example.com';
        const tpUserId = Array(64).fill(0).map(() => String.fromCharCode(97 + Math.floor(Math.random() * 26))).join('')
        const createdUser = await ThirdParty.manuallyCreateOrUpdateUser(
            "public", thirdPartyId, tpUserId, email, true
        );
        // expect(createdUser.status).toBe("OK");
        if (createdUser.status === "OK") {
            return {
...
export const createUsers = async () => {
    ...
    const plessEmailUsers = await measureTime("Passwordless users (with email) creation", createPasswordlessUsersWithEmail);

    const plessPhoneUsers = await measureTime("Passwordless users (with phone) creation", createPasswordlessUsersWithPhone);

    const tpUsers1 = await measureTime("ThirdParty users (google) creation", () => createThirdPartyUsers("google"));
