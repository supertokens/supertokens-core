    ...
    private static void checkAndStoreCode(Main main, TOTPStorage totpStorage, String userId, TOTPDevice[] devices,
        ...
        if (invalidOutOfN == N) {
            ...
            if (now - latestInvalidCodeCreatedTime < rateLimitResetTimeInMs) {
                ...
                // (because they are expired), and then user will again be able to
                // do extra login attempts (totp_max_attempts times).
                // But the cron running during cooldown of a user is somewhat rare.
                // And every period has enough entropy to make brute force attacks
                // infeasible, the code changes every period, and rate limiting will
                // kick in after totp_max_attempts number of attempts anyways.
