...
public class VerifySessionAPITest4_0 {
    @Rule
    ...
    public void checkThatrecipeUserIdIsNotPresentInV4() throws Exception {
        ...
        assertFalse(sessionInfo.has("recipeUserId"));
        ...
        assertEquals(response.get("status").getAsString(), "OK");
        assertTrue(response.get("session").getAsJsonObject().has("recipeUserId"));
        ...
        // [antiCsrfToken, sub, tId, sessionHandle, exp, iat, parentRefreshTokenHash1, key, refreshTokenHash1]
    ...
    public void successOutputCheckNewAccessTokenUpgradeToV5() throws Exception {
        ...
        // [antiCsrfToken, sub, sessionHandle, exp, iat, parentRefreshTokenHash1, key, refreshTokenHash1, tId, rsub]
        assertEquals(10, decodedJWT.getClaims().size());
        assertEquals("value", decodedJWT.getClaim("key").asString());
    ...
    public void testV4AccessTokenWithCustomtIdPayload() throws Exception {
        ...
        JsonObject userDataInJWT = new JsonObject();
        userDataInJWT.addProperty("key", "value");
        userDataInJWT.addProperty("rsub", userId);
        JsonObject userDataInDatabase = new JsonObject();
        userDataInDatabase.addProperty("key", "value");
...

        {
            String accessToken = sessionInfo.get("accessToken").getAsJsonObject().get("token").getAsString();
            DecodedJWT decodedJWT = com.auth0.jwt.JWT.decode(accessToken);
            assertEquals(decodedJWT.getClaim("rsub").asString(), userId);
        }
