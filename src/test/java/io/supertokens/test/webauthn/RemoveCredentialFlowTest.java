...
public class RemoveCredentialFlowTest {
    @Rule
    ...
    public void removeCredential() throws Exception {
        ...
        JsonObject removeCredentialResponse = Utils.removeCredential(process.getProcess(), userId, listCredentialsResponse.getAsJsonArray("credentials").get(0).getAsJsonObject().get("webauthnCredentialId").getAsString());
        assertEquals("OK", removeCredentialResponse.get("status").getAsString());


        removeCredentialResponse = Utils.removeCredential(process.getProcess(), userId, listCredentialsResponse.getAsJsonArray("credentials").get(0).getAsJsonObject().get("webauthnCredentialId").getAsString());
        assertEquals("CREDENTIAL_NOT_FOUND_ERROR", removeCredentialResponse.get("status").getAsString());

        listCredentialsResponse = Utils.listCredentials(process.getProcess(), userId);
        assertEquals("OK", listCredentialsResponse.get("status").getAsString());
        assertEquals(0, listCredentialsResponse.getAsJsonArray("credentials").size());
    }

    @Test
    public void removeCredentialWithUserIdMapping() throws Exception {
        String[] args = {"../"};
        TestingProcessManager.TestingProcess process = TestingProcessManager.start(args);
        FeatureFlagTestContent.getInstance(process.getProcess())
                .setKeyValue(FeatureFlagTestContent.ENABLED_FEATURES, new EE_FEATURES[]{
    ...
    @Test
    public void removeCredentialWithAccountLinking() throws Exception {
        String[] args = {"../"};
        TestingProcessManager.TestingProcess process = TestingProcessManager.start(args);
        FeatureFlagTestContent.getInstance(process.getProcess())
                .setKeyValue(FeatureFlagTestContent.ENABLED_FEATURES, new EE_FEATURES[]{
